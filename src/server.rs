...
impl ServerList {
        ...
        pub fn get_servers(num: usize) -> Result<Vec<Server>> {
                ...
                .map_err(|e| 
                        anyhow::anyhow!("Failed to retrieve speedtest configuration: {e:?}"))?;
        ...
        pub fn select_server(server_id: Option<String>) -> Result<Vec<Server>> {
                if let Some(id_str) = server_id {
                        let id = id_str.parse::<u32>().map_err(|_| {
                                anyhow::anyhow!(
                                        "Server ID must be a valid number"
                                )
                        })?;
                        let all_servers = Self::get_servers(TOP_X_NUM_SERVERS)?;
                        let filtered: Vec<Server> = all_servers
                                .into_iter()
                                .filter(|s| s.id == id)
                                .collect();
                        ...
                        if filtered.is_empty() {
                                anyhow::bail!(
                                        "Server with ID=\"{id}\" not found in available servers."
                                );
                        }
                        Ok(filtered)
                } else {
...
mod tests {
        ...
        fn test_ellipsize() {
                ...
                assert_eq!(
                        ellipsize("a very long sponsor name", 10),
                        "a very ..."
                );
        }
        ...
        fn test_select_server_invalid_id() {
                let result = ServerList::select_server(Some(
        ...
        fn test_select_server_valid_parse() {
                ...
                let result =
                        ServerList::select_server(Some("12345".to_string()));
